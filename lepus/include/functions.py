...
def add_alarm(server_id, tags, db_host, db_port, create_time, db_type, alarm_item, alarm_value, level, message,
    ...
    try:
        ...
        param = (
        server_id, tags, db_host, db_port, create_time, db_type, alarm_item, alarm_value, level, message, send_mail,
...
def check_if_ok(server_id, tags, db_host, db_port, create_time, db_type, alarm_item, alarm_value, message, send_mail,
                send_mail_to_list, send_sms, send_sms_to_list):
    conn = MySQLdb.connect(host=host, user=user, passwd=passwd, port=int(port), connect_timeout=5, charset='utf8')
    conn.select_db(dbname)
    curs = conn.cursor()
    if db_type == 'os':
        alarm_count = curs.execute(
            "select id from alarm_temp where ip='%s' and alarm_item='%s' ;" % (db_host, alarm_item))
        mysql_exec("delete from alarm_temp where ip='%s'  and alarm_item='%s' ;" % (db_host, alarm_item), '')
    else:
        alarm_count = curs.execute(
            "select id from alarm_temp where server_id=%s and db_type='%s' and alarm_item='%s' ;" % (
            server_id, db_type, alarm_item))
        mysql_exec("delete from alarm_temp where server_id=%s and db_type='%s' and alarm_item='%s' ;" % (
        server_id, db_type, alarm_item), '')

    if int(alarm_count) > 0:
        sql = "insert into alarm(server_id,tags,host,port,create_time,db_type,alarm_item,alarm_value,level,message,send_mail,send_mail_to_list,send_sms,send_sms_to_list) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
        param = (
        server_id, tags, db_host, db_port, create_time, db_type, alarm_item, alarm_value, 'ok', message, send_mail,
        send_mail_to_list, send_sms, send_sms_to_list)
...
def update_send_mail_status(server, db_type, alarm_item, send_mail, send_mail_max_count):
    ...
    if db_type == "os":
        alarm_count = curs.execute(
            "select id from alarm_temp where ip='%s' and db_type='%s' and alarm_item='%s' and alarm_type='mail' ;" % (
            server, db_type, alarm_item))
    else:
        alarm_count = curs.execute(
            "select id from alarm_temp where server_id=%s and db_type='%s' and alarm_item='%s' and alarm_type='mail' ;" % (
            server, db_type, alarm_item))
...
def update_send_sms_status(server, db_type, alarm_item, send_sms, send_sms_max_count):
    ...
    if db_type == "os":
        alarm_count = curs.execute(
            "select id from alarm_temp where ip='%s' and db_type='%s' and alarm_item='%s' and alarm_type='sms' ;" % (
            server, db_type, alarm_item))
    else:
        alarm_count = curs.execute(
            "select id from alarm_temp where server_id=%s and db_type='%s' and alarm_item='%s' and alarm_type='sms' ;" % (
            server, db_type, alarm_item))
...
def update_db_status_init(role, version, db_host, db_port, tags):
    try:
        ...
        conn.select_db(dbname)
        curs = conn.cursor()
        curs.execute("update db_status set role='%s',version='%s',tags='%s' where host='%s' and port='%s';" % (
        role, version, tags, db_host, db_port))
        conn.commit()
    except Exception, e:
...
def update_db_status(field, value, db_host, db_port, alarm_time, alarm_item, alarm_value, alarm_level):
    try:
        ...
        curs = conn.cursor()
        if db_port:
            curs.execute("update db_status set %s='%s',%s='%s' where host='%s' and port='%s';" % (
            field, value, field_tips, value_tips, db_host, db_port))
        else:
            curs.execute("update db_status set %s='%s',%s='%s' where host='%s';" % (
            field, value, field_tips, value_tips, db_host))
        conn.commit()
    except Exception, e:
...
#         n = re.search(r'\bvarname\s*\(\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)', line)
#         if n:
#             return n.groups()

def to_metrics(labels, metrics):
    metrics_list = []
...
def to_infludb(metrics):
    ...
    try:
        REVAL = cli.write_points(metrics)
        # print REVAL
