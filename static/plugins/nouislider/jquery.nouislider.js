...
(function($, UNDEF){
	...
	**/
		...
		function start ( event ) {
			...
			// Attach the move event views, while
		...
		function selfEnd( event ) {
			// Trigger the end views. Supply correct data using a
			// fake object that contains all required information;
			end({ data: { base: event.data.base, handle: event.data.handle } });
			// Stop propagation so that the tap views doesn't interfere;
		...
		function tap ( event ) {
			...
			// The event views might have rejected this event.
			if(!event) {
				return;
			}
		...
		// with a simple views. noUiSlider will use the internal
