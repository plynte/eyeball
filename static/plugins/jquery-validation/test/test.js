...
test("valid()", function() {
	...
	v = $('#testForm3').validate({ submitHandler: function() {
		ok( false, "Submit views was called" );
	}});
	ok( v.valid(), "No errors, must be valid and returning true, even with the submit views" );
	v.errorList = errorList;
	ok( !v.valid(), "One error, must be invalid, no call to submit views" );
});
...
test("showErrors() - custom views", function() {
	expect(5);
	var v = $('#testForm1').validate({
