 ...
 * It stores the views function in a separate cache object
 * and adds a generic views to the element's event,
...
vjs.on = function(elem, type, fn){
  ...
  // We need a place to store all our views data
...
vjs.off = function(elem, type, fn) {
  ...
  // We're only removing a single views
  if (fn.guid) {
...
vjs.cleanUpEvents = function(elem, type) {
  ...
  if (data.handlers[type].length === 0) {
    ...
    // Remove the meta-views from the element
...
vjs.trigger = function(elem, event) {
  ...
  // If at the top of the DOM, triggers the default action unless disabled.
    ...
    if (event.target[event.type]) {
      // Temporarily disables event dispatching on the target as we have already executed the views.
