...
from include import functions as func
from include import lepus_mysql as mysql
...
def check_mysql(host, port, username, password, server_id, tags):
    try:
        ...
        metrics = {
            ...
            "max_connect_errors": int(max_connect_errors),
            "open_files_limit": int(open_files_limit),
            "table_open_cache": int(table_open_cache),
            "max_tmp_tables": int(max_tmp_tables),
            "max_heap_table_size": int(max_heap_table_size),
            ...
            "threads_waits": threads_waits,
            "threads_created": int(threads_created),
            "threads_cached": int(threads_cached),
            "connections": int(connections),
            "aborted_clients": int(aborted_clients),
            "aborted_connects": int(aborted_connects),
            "connections_persecond": connections_persecond,
            "bytes_received_persecond": bytes_received_persecond,
            "bytes_sent_persecond": bytes_sent_persecond,
            "com_select_persecond": com_select_persecond,
            "com_insert_persecond": com_insert_persecond,
            "com_update_persecond": com_update_persecond,
            "com_delete_persecond": com_delete_persecond,
            "com_commit_persecond": com_commit_persecond,
            "com_rollback_persecond": com_rollback_persecond,
            "questions_persecond": questions_persecond,
            "queries_persecond": queries_persecond,
            "transaction_persecond": transaction_persecond,
            "created_tmp_tables_persecond": created_tmp_tables_persecond,
            "created_tmp_disk_tables_persecond": created_tmp_disk_tables_persecond,
            "created_tmp_files_persecond": created_tmp_files_persecond,
            "table_locks_immediate_persecond": table_locks_immediate_persecond,
            "table_locks_waited_persecond": table_locks_waited_persecond,
            "key_buffer_size": int(key_buffer_size),
            "sort_buffer_size": int(sort_buffer_size),
            "join_buffer_size": int(join_buffer_size),
            "key_blocks_not_flushed": int(key_blocks_not_flushed),
            "key_blocks_unused": int(key_blocks_unused),
            "key_blocks_used": int(key_blocks_used),
            "key_read_requests_persecond": key_read_requests_persecond,
            "key_reads_persecond": key_reads_persecond,
            "key_write_requests_persecond": key_write_requests_persecond,
            "key_writes_persecond": key_writes_persecond,
            "innodb_version": innodb_version,
            "innodb_buffer_pool_instances": int(innodb_buffer_pool_instances),
            "innodb_buffer_pool_size": int(innodb_buffer_pool_size),
            "innodb_doublewrite": innodb_doublewrite,
            "innodb_file_per_table": innodb_file_per_table,
            "innodb_flush_log_at_trx_commit": int(innodb_flush_log_at_trx_commit),
            "innodb_flush_method": innodb_flush_method,
            "innodb_force_recovery": int(innodb_force_recovery),
            "innodb_io_capacity": int(innodb_io_capacity),
            "innodb_read_io_threads": int(innodb_read_io_threads),
            "innodb_write_io_threads": int(innodb_write_io_threads),
            ...
            "innodb_buffer_pool_pages_flushed_persecond": innodb_buffer_pool_pages_flushed_persecond,
            "innodb_buffer_pool_read_requests_persecond": innodb_buffer_pool_read_requests_persecond,
            "innodb_buffer_pool_reads_persecond": innodb_buffer_pool_reads_persecond,
            "innodb_buffer_pool_write_requests_persecond": innodb_buffer_pool_write_requests_persecond,
            "innodb_rows_read_persecond": innodb_rows_read_persecond,
            "innodb_rows_inserted_persecond": innodb_rows_inserted_persecond,
            "innodb_rows_updated_persecond": innodb_rows_updated_persecond,
            "innodb_rows_deleted_persecond": innodb_rows_deleted_persecond,
            "query_cache_hitrate": float(query_cache_hitrate),
            "thread_cache_hitrate": float(thread_cache_hitrate),
            "key_buffer_read_rate": float(key_buffer_read_rate),
            "key_buffer_write_rate": float(key_buffer_write_rate),
            "key_blocks_used_rate": float(key_blocks_used_rate),
            "created_tmp_disk_tables_rate": float(created_tmp_disk_tables_rate),
            "connections_usage_rate": float(connections_usage_rate),
            "open_files_usage_rate": float(open_files_usage_rate),
            "open_tables_usage_rate": float(open_tables_usage_rate),
        }
        m1 = func.to_metrics(labels=labels, metrics=metrics)  # 返回指标数据 json格式
        func.to_infludb(m1)
        ...
        if processlist:
            for line in cur.fetchall():
                ...
                m2 = func.to_metrics(labels=labels2, metrics=metrics2)  # 返回指标数据 json格式
                # print "m2",m2
                func.to_infludb(m2)
        ...
        if connected:
            for line in cur.fetchall():
                ...
                metrics3 = {
                    "connect_server": line[0],
                    "connect_user": line[1],
                    "connect_db": line[2],
                    "connect_count": line[3],
                }
                m3 = func.to_metrics(labels=labels3, metrics=metrics3)  # 返回指标数据 json格式
                # print "m3",m3
                func.to_infludb(m3)
        ...
        if result:
            ...
            metrics4 = {
                ...
                "gtid_mode": result[2],
                "read_only": result[3],
                "master_server": result[4],
                "master_port": result[5],
                "slave_io_run": result[6],
                "slave_sql_run": result[7],
                "delay": result[8],
                "current_binlog_file": result[9],
                "current_binlog_pos": result[10],
                "master_binlog_file": result[11],
                "master_binlog_pos": result[12],
                "master_binlog_space": result[13],
            }
            m4 = func.to_metrics(labels=labels4, metrics=metrics4)  # 返回指标数据 json格式
            # print "m4", m4
            func.to_infludb(m4)
...
if __name__ == '__main__':
    main()
    # check_mysql(host='192.168.200.135',port=3306,username='lepus',password='ppM0ney',server_id=2,tags='lepus')
